...
export default function Navbar({ links, user }: TProps) {
  ...
  return (
    <Disclosure
      as='nav'
      className='bg-gray-800 fixed sm:absolute w-screen z-20'>
      {({ open }) => (
        <>
            ...
            <div className='relative flex items-center justify-between h-16'>
              ...
              <div className='flex-1 flex items-center justify-center sm:items-stretch sm:justify-start'>
                <div className='flex-shrink-0 flex items-center'>
                  <img
                    className='block h-8 w-auto sm:h-5 lg:h-8'
                    src={logo}
                    alt='Delphos-IQ logo'
                  />
                </div>
              ...
              <div className='absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0'>
                ...
                <Menu
                  ...
                  <div>
                    <Menu.Button className='bg-gray-800 max-w-[40px] max-h-[40px] flex text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white'>
                      <span className='sr-only'>Open user menu</span>
                      <img
                        className='h-8 sm:h-3 lg:h-8 w-full rounded-full'
                        src={ManPNG}
                        alt='profile'
                      />
                    </Menu.Button>
          ...
          <Transition.Child
            ...
            <Disclosure.Panel className='sm:hidden'>
              <nav className='sm:hidden px-2 pt-2 pb-3 h-[calc(100vh_-_theme(height.20))] w-full fixed bg-primary'>
                <ul className='list-none flex flex-col justify-center space-y-1'>
                  {links.map((link, index) => (
                    <li key={index}>
                      <span className='text-white'>{link.name}</span>
                    </li>
